    ...
    if(mavlink_parse_char(MAVLINK_COMM_0, c, &msg, &status)) {
      switch(msg.msgid) {
        ...
        case MAVLINK_MSG_ID_GPS_RAW_INT:     
          ...
          gps_time_msec = (uint32_t)(mavlink_msg_gps_raw_int_get_time_usec(&msg) / 1000);
          ...
          logger->debug_print(Logger::LOG_MAV_GPS, (char *)"MAVLINK_MSG_ID_GPS_RAW_INT: msec: %ld fixtype: %d, sats: %d, hdop: %d, speed: %ld, alt: %ld lat: %ld lon: %ld", gps_time_msec, gps_fixtype, gps_satellites_visible, gps_hdop, gps_speed, gps_altitude, gps_latitude, gps_longitude);
          armed_bit = (base_mode >> 7) & 1;
          if(armed_bit) {
        ...
        case MAVLINK_MSG_ID_VFR_HUD:                              
          ...
          logger->debug_print(Logger::LOG_MAV_VFR, (char *)"MAVLINK_MSG_ID_VFR_HUD: groundspeed: %f, airspeed: %f, heading: %d, throttle: %d, alt: %f, climbrate: %f", mavlink_msg_vfr_hud_get_groundspeed(&msg), mavlink_msg_vfr_hud_get_airspeed(&msg), mavlink_msg_vfr_hud_get_heading(&msg), mavlink_msg_vfr_hud_get_throttle(&msg), mavlink_msg_vfr_hud_get_alt(&msg), mavlink_msg_vfr_hud_get_climb(&msg));
